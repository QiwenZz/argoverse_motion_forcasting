...
class baselineLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        _, (last_hidden, last_cell) = self.lstm_46(x, (last_hidden, last_cell))
        out_47 = torch.unsqueeze(self.linear_47(last_hidden[1]),1)
        _, (last_hidden, last_cell) = self.lstm_47(x, (last_hidden, last_cell))
        out_48 = torch.unsqueeze(self.linear_48(last_hidden[1]),1)
        _, (last_hidden, last_cell) = self.lstm_48(x, (last_hidden, last_cell))
        out_49 = torch.unsqueeze(self.linear_49(last_hidden[1]),1)
        _, (last_hidden, last_cell) = self.lstm_49(x, (last_hidden, last_cell))
